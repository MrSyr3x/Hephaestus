...
// ─── Base dependencies ────────────────────────────────────────────────────────
// Fetched by Sandpack in the browser on every single preview, whether the
// generated app uses them or not — so this is a safety net for packages
// Gemini forgets to declare in its own "dependencies" JSON (see SYSTEM_PROMPT
// in app/api/gen-ai-code/route.ts), not a place to default-include anything
// that's merely plausible. Previously included the 6 @radix-ui/* packages,
// axios, and react-is — the system prompt never tells Gemini to use Radix
// (rule 4 says Tailwind for all styling) or axios (fetch is already global),
// and react-is is an internal package application code doesn't import
// directly. Cutting those ~8 packages measurably shrinks what every preview
// has to fetch before it can render.
    ...
    >
      ...
      <div className="relative flex-1 overflow-hidden h-full">
        {(isGenerating || isImproving) && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-6 bg-[#1a1815]/85 backdrop-blur-sm">
            <RingLoader color="#DD8967" size={64} speedMultiplier={0.8} />
