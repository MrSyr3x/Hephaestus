...
export function ChatPanel({
  ...
  return (
    <div className="flex w-[320px] shrink-0 flex-col bg-[#1d1b18]">
      {/* Header */}
      <div className="flex items-center justify-between gap-2 border-b border-white/6 px-2 py-3">
        <BlueTitle className="min-w-0 truncate">{appTitle}</BlueTitle>
                    ...
                    <div className="min-w-0 rounded-2xl rounded-tl-sm bg-white/5 px-3.5 py-2.5">
                      {isLiveStream && !msg.content ? (
                        ...
                        <div className="flex items-center gap-2">
                          <Wand2 className="h-3 w-3 shrink-0 text-[#DD8967]/60 animate-pulse" />
                        ...
                        <div>
                          <div className="mb-1.5 flex items-center gap-1.5">
                            <Wand2 className="h-3 w-3 shrink-0 text-[#DD8967]/60" />
                            <span className="text-[10px] font-medium uppercase tracking-wider text-[#DD8967]/50">
                              Agent reasoning
                            </span>
                          </div>
                          <p className="text-[12px] leading-relaxed text-white/35 wrap-break-word">
                            {msg.content}
                            <span className="ml-0.5 inline-block h-3 w-0.5 animate-[blink_1s_ease-in-out_infinite] bg-[#DD8967]/60 align-middle" />
                          </p>
                        </div>
                      ) : (
                        // Normal completed assistant message
                        <div className="prose prose-sm prose-invert max-w-none wrap-break-word text-[13px] leading-relaxed text-white/70 [&_code]:rounded [&_code]:bg-white/10 [&_code]:px-1 [&_code]:py-0.5 [&_code]:text-[#E8A488]/80 [&_code]:text-xs [&_code]:break-all [&_li]:my-0.5 [&_p]:my-1 [&_pre]:overflow-x-auto! [&_pre]:whitespace-pre-wrap! [&_ul]:my-1">
                  ...
                  {statusLog.map((step, i) => (
                    <div key={i} className="flex items-center gap-2.5">
                      <div className="flex h-4 w-4 shrink-0 items-center justify-center">
                        {step.status === "running" ? (
                          <Loader2 className="h-3 w-3 animate-spin text-[#DD8967]/80" />
                        ) : (
                          <svg
